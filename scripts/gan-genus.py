...
def combine_etymology(triplet, parts):
            ...
            if part == "Root":
                ...
                if part in ("Language", "Gender", "Part"):
                    if not pd.isna(w[part][key]):
                        etymology_html += (
                            '<span class="glossary">' + str(w[part][key]) + "</span>"
                        )
    ...
    for index in reversed(range(len(triplet))):
        try:
            if "Explanation" in parts[index] and (
                not pd.isna(parts[index]["Explanation"][triplet[index]])
            ):
                try:
                    hint += parts[index]["Explanation"][triplet[index]]
                except Exception as e:
                    eprint(
                        "WARNING: Error in column Explanation:\n"
                        + parts[index]["Explanation"][triplet[index]],
                        "\t",
                        e,
                    )
                    quit(1)
...
if __name__ == "__main__":
    ...
    opt_parser.add_argument(
        "-1", "--first", help='First Excel file in "GAN" format', required=True
    )

    opt_parser.add_argument(
        "-2", "--second", help='Second Excel file in "GAN" format', required=True
    )

    opt_parser.add_argument("-3", "--third", help='Third Excel file in "GAN" format')

    opt_parser.add_argument("-o", "--outdir", help="Output directory", required=True)

    opt_parser.add_argument(
        "-p", "--prefix", help="Output basename [default: 'gan']", default="gan"
    )
    opt_parser.add_argument(
        "-c",
        "--connector",
    ...
    if opt.third is not None:
        ...
        for triplet in permutations:
            ...
            combinedEtym, fullEtym, rawEtym = combine_etymology(
                triplet, [prefixes, mids, suffixes]
            )
